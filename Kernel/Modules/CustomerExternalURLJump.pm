...
package Kernel::Modules::CustomerExternalURLJump;
...
sub Run {
    ...
    my $URLIsConfigured;
    my $ExternalURLJumpConfigs = $FrontendNavigationConfig->{CustomerExternalURLJump} // {};
    ...
    for my $ExternalURLJumpConfigKey ( sort keys %{$ExternalURLJumpConfigs} ) {
        ...
        for my $Element ( @{ $ExternalURLJumpConfigs->{$ExternalURLJumpConfigKey} // [] } ) {
            ...
            if ( $Link !~ m{\AAction=CustomerExternalURLJump;URL=(.+)} ) {
                $LogObject->Log(
                    Priority => 'error',
                    Message  =>
